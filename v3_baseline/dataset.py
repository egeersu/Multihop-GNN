...
#             text_set = text_set[20000:29001]
        ...
        print("Sample from", text_set[0]['id'], " to ", text_set[-1]['id'])
...
#                 self.answer_mask_set.append(answer_mask)
        ...
        print("Check set size:",len(self.graph_set), len(self.text_set))
    ...
    def get_batch_label(self, text_batch):
        batch_yy = []
        for d in text_batch:
            batch_yy.append(d['candidates'].index(d["answer"]))
        batch_yy = torch.tensor(batch_yy, dtype=torch.long)
        return batch_yy
