...
max_edges = 1500 # For memory limitation (1500 for 6G GPU RAM)
...
def get_args():
    ...
    parser.add_argument('--dataset', type=str, default=None, help='specify generate graph for which dataset.')
...
#     parser.add_argument('--epoch-checkpoints', action='store_true', help='store all epoch checkpoints')
    ...
    # GNN Setting
    parser.add_argument('--max-nodes', type=int, default=300, help='if use GPU, need cuda.')
    parser.add_argument('--max-edges', type=int, default=1500, help='if use GPU, need cuda.')
    ...
    parser.add_argument('--learning-rater', default=1e-4, type=float, help='learning rate')
